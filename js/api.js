...
const EarthquakeAPI = (() => {
  ...
  const regionPresets = {
    ...
    pacific: {
      label: '環太平洋',
      bounds: { minlat: -60, maxlat: 65, minlon: 100, maxlon: -60 },
    },
    southeast_asia: {
      label: '東南アジア',
  ...
  async function search(params) {
    ...
    // 地域フィルタ
    if (params.minlat) query.set('minlatitude', params.minlat);
    if (params.maxlat) query.set('maxlatitude', params.maxlat);
    if (params.minlon) query.set('minlongitude', params.minlon);
    if (params.maxlon) query.set('maxlongitude', params.maxlon);
